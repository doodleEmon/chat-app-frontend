...
export default function Home() {
  ...
  const handleClickChat = (m: number) => {
    setIsClicked(true);
    setClickedValue(m);
  }
  ...
  return (
    <div className="w-full h-[calc(100vh-4rem)] flex">
        ...
        <div className="h-full overflow-hidden overflow-y-scroll mt-4 mb-24 space-y-2 scrollbar-thin">
          {
            messages.map((m, index) => (
              <div key={index} onClick={() => handleClickChat(m)} className="flex items-center gap-x-4 py-2 px-3 hover:bg-slate-700 cursor-pointer">
      ...
      <div className="w-[80%] h-full flex items-center justify-center p-4">
        {
          isClicked && clickedValue > 0 ? <ChatContainer value={clickedValue} /> : (
